...
for path_idx, uid in tqdm(enumerate(data_list)):
    ...
    _, _, surf_ncs, surf_ukvs, surf_vkvs, edge_ncs, _, _, _, _, _, _, _, _ = data.values()
    ...
    for np_bit, np_real, ukv, vkv in zip(data_bits, data, surf_ukvs, surf_vkvs):
        total += 1

        # Reshape the array to a 2D array
        np_bit = np_bit.reshape(-1, 4)
        ...
        if prev_len < len(unique_hash):
            ...
            if not args.edge:
              unique_ukv.append(ukv)
              unique_vkv.append(vkv)
        else:
            continue
...
if args.edge:
    ...
    save_path = args.list.split('.')[0] + '_surface_pw.pkl'
    with open(save_path, "wb") as tf:
        pickle.dump(unique_data, tf)
    save_path = args.list.split('.')[0] + '_surface_ukv.pkl'
    with open(save_path, "wb") as tf:
        pickle.dump(unique_ukv, tf)
    save_path = args.list.split('.')[0] + '_surface_vkv.pkl'
    with open(save_path, "wb") as tf:
        pickle.dump(unique_vkv, tf)
