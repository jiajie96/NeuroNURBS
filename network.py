...
class nurbs_vae(nn.Module):
    def __init__(self, config):
        super(nurbs_vae, self).__init__()
        self.config = config
        self.dropout = nn.Dropout(p=0.1)
        self.max_ctrlPts = self.config.max_ctrlPts
        self.kv_length = self.config.max_kv
        self.dim = self.config.feature_dim
        self.embed_dim = 64

        self.x_embed = nn.Sequential(
            Rearrange('b c h w -> b (h w) (c)'), # bs, 4, 10, 10 --> bs, (100), (4)
            nn.Linear(4, self.embed_dim), #--> bs, (100), (512)
        ...
        self.kv_embed = nn.Sequential(
            nn.Linear(self.kv_length, self.embed_dim), #--> bs, (100), (10)--> bs, 100, (512)
            nn.LayerNorm(self.embed_dim),
            nn.SiLU(),
            nn.Linear(self.embed_dim, self.embed_dim),
        ) 

        # self.vkv_embed = nn.Sequential(
        #     nn.Linear(self.kv_length, self.embed_dim), #--> bs, (100), (10)--> bs, 100, (512)
        #     nn.LayerNorm(self.embed_dim),
        #     nn.SiLU(),
        #     nn.Linear(self.embed_dim, self.embed_dim),
        ...
        self.pos_embedding = nn.Parameter(torch.randn(1, self.max_ctrlPts**2, self.embed_dim)) # --> bs, (100), (512)

        layer = nn.TransformerEncoderLayer(d_model=self.embed_dim, nhead=4, norm_first=True,
                                                   dim_feedforward=256, dropout=0.1)
        self.net = nn.TransformerEncoder(layer, 8, nn.LayerNorm(self.embed_dim))

        self.to_latent = nn.Sequential(
            nn.LayerNorm(self.embed_dim),
            nn.Linear(self.embed_dim, self.dim*2)
        )

        self.vector_quantization = VectorQuantizer(
            n_e=512, e_dim=self.dim, beta=0.25)

        self.linear_up_latent = nn.Sequential(
            ...
            nn.LayerNorm(1048),
            nn.SiLU(),
            nn.Linear(1048, self.max_ctrlPts**2 * self.dim),
        )
        self.linear_down_x0 = nn.Sequential(
            nn.Linear(self.dim, 256),
            nn.LayerNorm(256),
            nn.SiLU(),
            nn.Linear(256, 4),
            Rearrange('b (h w) (c)  -> b c (h) (w) ', h=self.max_ctrlPts, w=self.max_ctrlPts)
        )
        self.linear_down_x1 = nn.Sequential(
            nn.Linear(self.dim, 256),
            nn.LayerNorm(256),
            nn.SiLU(),
            nn.Linear(256, self.kv_length)
        )
        self.linear_down_x2 = nn.Sequential(
            nn.Linear(self.dim, 256),
            nn.LayerNorm(256),
            nn.SiLU(),
            nn.Linear(256, self.kv_length)
        ...
        # _, out, _, _, _ = self.vector_quantization(out)

    def encode(self, x, ukv, vkv):
        x_embedded = self.x_embed(x) # bs, 4, 10, 10 --> bs, (100), (512)
        ukv = ukv.unsqueeze(-2).repeat(1,self.max_ctrlPts**2,1) #--> bs, (10)--> bs, 100, (10)
        vkv = vkv.unsqueeze(-2).repeat(1,self.max_ctrlPts**2,1) #--> bs, (10)--> bs, 100, (10)
        ukv_embedded = self.kv_embed(ukv)
        vkv_embedded = self.kv_embed(vkv)
        pnt_mask = x.flatten(2,3).permute(0,2,1)[:,:,3] == -1

        tokens = x_embedded + self.pos_embedding[:, :] + ukv_embedded + vkv_embedded
        # print('tokens', tokens.shape)
        out = self.net(tokens.permute(1,0,2), src_key_padding_mask=pnt_mask).transpose(0,1) # bs, 100, (48)
        out = out.mean(dim = 1) #--> bs, 48  
        # print('out', out.shape)
    ...
    def decode(self, latent):
        latent = self.linear_up_latent(latent) # LS (bs, 64) --> data (bs, 400* 64)
        latent = latent.view(latent.size(0), self.max_ctrlPts**2, self.dim)  # data --> data (bs, 400, 64)
        
        decoded_x0 = self.linear_down_x0(latent) # data  (bs, 25, 64) --> data (bs, 25, 4*4*4) --> data (bs, 4, 20, 20)
        decoded_x1 = self.linear_down_x1(latent).mean(dim = 1)
        decoded_x2 = self.linear_down_x2(latent).mean(dim = 1)
        return decoded_x0, decoded_x1, decoded_x2

    def forward(self, x0, x1, x2):
        mu, sigma = self.encode(x0, x1, x2)
        z = self.reparameterize(mu, sigma)
        decoded_x0, decoded_x1, decoded_x2 = self.decode(z)        #
        kld = 0.5 * torch.sum(
            torch.pow(mu, 2) +
            torch.pow(sigma, 2) -
            torch.log(1e-8 + torch.pow(sigma, 2)) - 1
        ) / (decoded_x0.size(0) * 5 * 5)

        # embedding_loss, z_q, perplexity, _, _ = self.vector_quantization(z)
        # print(z.shape)
        return  decoded_x0, decoded_x1, decoded_x2, kld
      
# Here before added by NurbsGen
