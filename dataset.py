...
def filter_data(data):
    ...
    with open(data_path, "rb") as tf:
        data = pickle.load(tf)
    _, _, _, _, _, _, _, _, _, faceEdge_adj, surf_bbox, edge_bbox, _, _ = data.values()   
...
def load_data(input_data, input_list, validate, args, test = False):
    # Filter data list
...
class SurfData(torch.utils.data.Dataset):
        ...
        if self.validate: 
            ...
            for uid in data_list:
                ...
                with open(path, "rb") as tf:
                    data = pickle.load(tf)
                _, _, surf_pw, surf_ukvs, surf_vkvs, _, _, _, _, _, _, _, _, _ = data.values() 
                pws.append(surf_pw)
                ukvs.append(surf_ukvs)
                vkvs.append(surf_vkvs)
            self.pws = np.vstack(pws)
            self.ukvs = np.vstack(ukvs)
            self.vkvs = np.vstack(vkvs)

        # Load training data (deduplicated)
        else:
            print('Loading training data...')
            with open(input_list[:-4] + '_pw.pkl', "rb") as tf:
                self.pws = pickle.load(tf)
            with open(input_list[:-4] + '_ukv.pkl', "rb") as tf:
                self.ukvs = pickle.load(tf)
            with open(input_list[:-4] + '_vkv.pkl', "rb") as tf:
                self.vkvs = pickle.load(tf)
    ...
    def __getitem__(self, index):
        surf_pw = self.pws[index]
        surf_ukv = self.ukvs[index]
        surf_vkv = self.vkvs[index]
        if np.random.rand()>0.5 and self.aug:
            assert surf_pw.shape[-2] == surf_pw.shape[-3]
            max_ctrl_setting = surf_pw.shape[-2]
            aug_pw = remove_pw_pad(surf_pw)
            aug_ctrlPts = aug_pw[:, :, :3]
            aug_weight = aug_pw[:, :, 3:]
            for axis in ['x', 'y', 'z']:
                angle = random.choice([90, 180, 270])
                aug_ctrlPts = rotate_point_cloud(aug_ctrlPts.reshape(-1, 3), angle, axis).reshape(aug_ctrlPts.shape[0], aug_ctrlPts.shape[1], 3)
            aug_pw = np.concatenate([aug_ctrlPts, aug_weight],-1)
            surf_pw = np.ones((max_ctrl_setting, max_ctrl_setting, 4)) * (-1)
            surf_pw[:aug_pw.shape[0], :aug_pw.shape[1],:] = aug_pw
        return torch.FloatTensor(surf_pw), torch.FloatTensor(surf_ukv), torch.FloatTensor(surf_vkv)
    ...
    def __init__(self, input_data, input_list, validate=False, aug=False): 
        ...
        if self.validate: 
            ...
            for uid in tqdm(data_list):
                ...
                _, _, _ , _ , _ , edge_u, _, _, _, _, _, _, _, _ = data.values() 
                datas.append(edge_u)
...
class SurfPosData(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        _, _, _, _, _, _, _, _, _, _, surf_pos, _, _, _ = data.values()
...
class SurfZData(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        with open(data_path, "rb") as tf:
            data = pickle.load(tf)
        _, _, surf_ncs, surf_ukvs, surf_vkvs, _, _, _, _, _, surf_pos, _, _, _ = data.values()
        ...
        if random_num>0.5 and self.aug:
            # Get all eight corners
            surfpos_corners = bbox_corners(surf_pos)
            max_ctrl_setting = surf_ncs.shape[-2]
            aug_ctrlPts = surf_ncs[:, :, :, :3]
            aug_weight = surf_ncs[:, :, :, 3:]

            # Random rotation
            for axis in ['x', 'y', 'z']:
                angle = random.choice([90, 180, 270]) 
                surfpos_corners = rotate_axis(surfpos_corners, angle, axis, normalized=True)
                aug_ctrlPts = rotate_axis(aug_ctrlPts, angle, axis, normalized=False)

            surf_ncs = np.concatenate([aug_ctrlPts, aug_weight], -1)
            for i in range(surf_ncs.shape[0]):
                for _u in range(surf_ncs.shape[1]):
                    for _v in range(surf_ncs.shape[2]):
                        if surf_ncs[i, _u, _v, 3] <-0.9:
                            surf_ncs[i, _u, _v, :3] = [-1, -1, -1]
        ...
        random_indices = np.random.permutation(surf_pos.shape[0])
        surf_pos = surf_pos[random_indices]
        surf_ukvs = surf_ukvs[random_indices]
        surf_vkvs = surf_vkvs[random_indices]
        surf_ncs = surf_ncs[random_indices]
    
        # Pad data
        surf_pos, surf_mask = pad_minusone(surf_pos, self.max_face, return_mask=True)
        surf_ncs = pad_minusone(surf_ncs, self.max_face)
        surf_ukvs = pad_minusone(surf_ukvs, self.max_face)
        surf_vkvs = pad_minusone(surf_vkvs, self.max_face)
        ...
        if data_class is not None:
            return (
                ...
                torch.FloatTensor(surf_ukvs),
                torch.FloatTensor(surf_vkvs),
...
class EdgePosData(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        _, _, surf_ncs, surf_ukvs, surf_vkvs, _, _, _, _, faceEdge_adj, surf_pos, edge_pos, _, _ = data.values()
        ...
        surf_ukvs = pad_minusone(surf_ukvs, self.max_face)
        surf_vkvs = pad_minusone(surf_vkvs, self.max_face)
        edge_pos = pad_minusone(edge_pos, self.max_face)
...
class EdgeZData(torch.utils.data.Dataset):
    ...
    def __init__(self, input_data, input_list, validate=False, aug=False, args=None, test=False): 
        self.max_face = args.max_face
        self.max_edge = args.max_edge
        self.bbox_scaled = args.bbox_scaled
        self.aug = aug
        self.data = []
        # Load data 
        self.data = load_data(input_data, input_list, validate, args, test)
        # Inflate furniture x50 times for training
        if len(self.data)<2000 and not validate:
            self.data = self.data*50
        return 
    ...
    def __getitem__(self, index):
        ...
        _, _, surf_ncs,surf_ukvs, surf_vkvs, edge_ncs, corner_wcs, _, _, faceEdge_adj, surf_pos, edge_pos, _, _ = data.values()
        ...
        for pos, ncs, vert in zip(edge_pos_duplicated, edge_ncs_duplicated, vertex_pos_duplicated):
            ...
            pos, mask = pad_minusone(pos, self.max_edge, return_mask=True)
            ncs = pad_minusone(ncs, self.max_edge)
            vert = pad_minusone(vert, self.max_edge)
        ...
        # Padding
        surf_pos = pad_minusone(surf_pos, self.max_face)
        surf_ncs = pad_minusone(surf_ncs, self.max_face)
        surf_ukvs = pad_minusone(surf_ukvs, self.max_face)
        surf_vkvs = pad_minusone(surf_vkvs, self.max_face)
        edge_pos = pad_minusone(edge_pos, self.max_face)
        edge_ncs = pad_minusone(edge_ncs, self.max_face)
        vertex_pos = pad_minusone(vertex_pos, self.max_face)
...
class testData(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        surf_wcs, edge_wcs, surf_ncs, surf_ukvs, surf_vkvs, edge_ncs, corner_wcs, edgeFace_adj, edgeCorner_adj, faceEdge_adj, surf_pos, edge_pos, _, _ = data.values()

        # print(surf_wcs.shape)
        # print(faceEdge_adj.shape)
        # Mating duplication
        edge_pos_duplicated = []
        vertex_pos_duplicated = []
        edge_ncs_duplicated = []
        ...
        if data_class is not None:
            return (
                torch.FloatTensor(surf_wcs), 
                torch.FloatTensor(edge_wcs), 
                torch.FloatTensor(edge_ncs), 
                torch.FloatTensor(edge_pos), 
                torch.BoolTensor(edge_mask),
                torch.FloatTensor(surf_ncs), 
                ...
                torch.FloatTensor(surf_pos), 
                torch.FloatTensor(vertex_pos), 
                torch.FloatTensor(corner_wcs), 
                # torch.Tensor(faceEdge_adj), 
                # torch.Tensor(edgeCorner_adj), 
                torch.LongTensor([data_class+1]) # add 1, class 0 = uncond (furniture)
            )
        else:
            return (
