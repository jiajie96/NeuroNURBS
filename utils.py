...
def scale_bboxes(bboxes, scale_range, min_values=None, max_values=None):
    if min_values is None: 
        min_values = torch.amin(bboxes)
    if max_values is None:
        max_values = torch.amax(bboxes)
...
def compute_bbox_centers_and_sizes(min_corners, max_corners):
    ...
    if isinstance(min_corners, np.ndarray) and isinstance(max_corners, np.ndarray):
        return_numpy = True
    # Ensure that the inputs are multi-dimensional arrays
    if len(min_corners.shape) == 1:
        min_corners = min_corners[None, :]
    if len(max_corners.shape) == 1:
        max_corners = max_corners[None, :]
    min_corners, max_corners = torch.tensor(min_corners), torch.tensor(max_corners)
    # Ensure that the min_corner is actually the minimum corner and the max_corner is the maximum corner
    min_corners, max_corners = torch.minimum(min_corners, max_corners), torch.maximum(min_corners, max_corners)
    # Calculate the centers
    centers = (min_corners + max_corners) / 2
    # Calculate the sizes
    sizes = torch.amax(max_corners - min_corners, dim=-1)

    if return_numpy:
        centers = centers.numpy(force=True)
        sizes = sizes.numpy(force=True)
...
def pad_minusone(x, max_len, return_mask=False):
    keys = np.ones(len(x))
    padding = np.zeros((max_len-len(x))).astype(int)  
    mask = 1-np.concatenate([keys, padding]) == 1  
    padding = np.ones((max_len-len(x), *x.shape[1:])) * (-1)
...
def get_args_vae():
    ...
    # model parameters
    parser.add_argument('--max_ctrlPts', type=int, default=10, help='max_ctrlPts') 
    parser.add_argument('--max_kv', type=int, default=10, help='max_kv')  
    parser.add_argument('--feature_dim', type=int, default=48, help='feature_dim')  
...
def get_args_ldm():
    ...
    parser.add_argument('--surfvae', type=str, default='proj_log/deepcad_vae_surf.pt', 
                        help='Path to pretrained surface vae weights')  
    parser.add_argument('--edgevae', type=str, default='proj_log/deepcad_vae_edge.pt', 
    ...
    parser.add_argument("--finetune",  action='store_true', help='Finetune from existing weights')
    parser.add_argument("--weight",  type=str, default='proj_log/deepcad_ldm_edgez.pt', help='Weight path when finetuning')  
  
    #VAE parameters 
    parser.add_argument('--max_ctrlPts', type=int, default=10, help='maximum ctrlPts')
    parser.add_argument('--max_kv', type=int, default=10, help='maximum length of knot vectors')
    parser.add_argument('--feature_dim', type=int, default=48, help='maximum length of knot vectors')
...
def joint_optimize(surf_ncs, edge_ncs, surfPos, unique_vertices, EdgeVertexAdj, FaceEdgeAdj, num_edge, num_surf, use_local_cd=True):
    """
    ...
    bbox_threshold_min = []
    bbox_threshold_max = []   
    surf_ncs_pc = surf_ncs[:,:,:,:3]
    surf_ncs_w = (surf_ncs[:,:,:,3:] +1)/2
    
    for edges_perface, ncs, bbox in zip(face_edges, surf_ncs_pc, surfPos):
        surf_center, surf_scale = compute_bbox_centers_and_sizes(bbox[0:3], bbox[3:])
        edges_perface_flat = edges_perface.reshape(-1, 3).detach().cpu().numpy()
        min_point, max_point = get_bbox_minmax(edges_perface_flat)
        edge_center, edge_scale = compute_bbox_centers_and_sizes(min_point, max_point)
    ...
    surf_wcs_init = np.concatenate([surf_wcs_init, surf_ncs_w], -1)
    ...
    #     surf_updated = surf_scale*surf_pc + surf_offset 

    #     surf_loss = 0
    #     for surf_pnt, edge_pnts, weights in zip(surf_updated, face_edges, surf_w):

    #         non_zero_idx = torch.nonzero(weights > 0.1).squeeze()
    #         surf_pnt = surf_pnt[list(set(non_zero_idx[0])),:, :]
    #         surf_pnt = surf_pnt[:,list(set(non_zero_idx[1])), :]

    #         surf_pnt = surf_pnt.reshape(-1,3)
    #         edge_pnts = edge_pnts.reshape(-1,3).detach()
            
    #         surf_loss  += loss_func(edge_pnts.unsqueeze(0), surf_pnt.unsqueeze(0))[0]
    ...
    # surf_updated = torch.cat([surf_updated, surf_w], dim=-1)
    # surf_wcs = surf_updated.detach().cpu().numpy()
    surf_wcs = surf.detach().cpu().numpy()
...
from helpers.construct_nurbs import *

def remove_pw_pad(source_pw, threshold=-0.98):
    non_zero_idx = np.where(source_pw[:,:,3]>threshold)
    unpadded_pw = source_pw[list(set(non_zero_idx[0])),:, :]
    unpadded_pw = unpadded_pw[:,list(set(non_zero_idx[1])), :]
    return unpadded_pw

def construct_brep(surf_wcs, surf_ukvs, surf_vkvs, edge_wcs, FaceEdgeAdj, EdgeVertexAdj):
    """
    ...
    # Fit surface bspline
    recon_faces = []  
    for _ctrlPts, _u_knotvector, _v_knotvector in zip(surf_wcs, surf_ukvs, surf_vkvs):
        # approx_face =  GeomAPI_PointsToBSplineSurface(uv_points_array, 3, 8, GeomAbs_C2, 5e-2).Surface() 
        surf = construct_Nurbs(_ctrlPts, _u_knotvector, _v_knotvector, tolerance = False)
        recon_faces.append(surf)
