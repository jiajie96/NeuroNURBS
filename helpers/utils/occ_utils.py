...
from OCC.Core.TopAbs import TopAbs_FACE, TopAbs_REVERSED, TopAbs_EDGE, TopAbs_VERTEX, TopAbs_SHELL
...
import helpers.utils.occ_topology
import helpers.utils.geom_utils
...
def edges_at_vertex(vert, face):
    f_topo = utils.occ_topology.Topo(face)
    v_edges = [edge for edge in f_topo.edges_from_vertex(vert)]
    return v_edges


def list_verts_ordered(face):
    f_util = face.Face(face)
    w_util = utils.occ_topology.WireExplorer(next(f_util.topo.wires()))
...
if __name__ == '__main__':
    ...
    pnts = utils.geom_utils.points_inside_rect([0, 0, 0], [10, 0, 0], [10, 10, 0], [0, 10, 0])
    for pnt in pnts:
